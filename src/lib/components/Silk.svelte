<script lang="ts">
	...
	const current = $derived({ speed, scale, color, backgroundColor, noiseIntensity, rotation });
...
void main() {
  ...
  vec3 col = mix(uBackgroundColor, uColor, pattern) - rnd / 15.0 * uNoiseIntensity;
...
`;
	...
	onMount(() => {
		...
		const uniforms = {
			...
			uBackgroundColor: { value: new THREE.Color(...hexToRgb(current.backgroundColor)) },
		...
		const animate = () => {
			...
			uniforms.uBackgroundColor.value.setRGB(...hexToRgb(c.backgroundColor));
		...
		return () => {
			...
			renderer.dispose();
			// eslint-disable-next-line svelte/no-dom-manipulating
			if ((renderer.domElement as HTMLCanvasElement).parentNode === container) container.removeChild(renderer.domElement as HTMLCanvasElement);
		};
	});
</script>
